...
const CUTIN_DURATION = 2000;       // カットイン中はレース時間を止める
const FINISH_WINDOW = 1200;
const FINISH_OVERLAY_DURATION = 1000;       // ゴール順表示の時間幅
...
function updateLiveRank(raceState){
  ...
  list.innerHTML=ranked.map((s,i)=>`<div class="live-rank-row"><span>${i+1}位</span><b>${s.runner.horseName}</b></div>`).join("");
}
...
async function handleFinish(){
  if(state.showingFinish) return;
  const candidate = state.lockedOrder.find((r)=>{
    if(state.finishIds.includes(r.id)) return false;
    const s = state.raceState.find(x=>x.runner.id===r.id);
    return s && state.activeRaceTime >= s.finishAt;
  });
  if(!candidate) return;

  state.showingFinish = true;
  state.paused = true;

  const rank = state.lockedOrder.indexOf(candidate) + 1;
  const s = state.raceState.find(x=>x.runner.id===candidate.id);
  if(s){
    s.progress = 1;
    s.finished = true;
  }
  state.finishIds.push(candidate.id);

  const idx=getRunnerIndex(candidate);
  setDotPosition(idx,1,idx);
  const dot=$(`dot-${idx}`);
  if(dot){
    dot.classList.add("finished");
    dot.querySelector(".mini-name").textContent=`🏁 ${rank}着 ${candidate.horseName}`;
  }
  updateGoalBoard(state.finishIds);
  await showFinishOverlay(rank, candidate);
...
async function showFinishOverlay(rank,runner){
  ...
  await sleep(FINISH_OVERLAY_DURATION);
